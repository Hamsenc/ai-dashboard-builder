...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --timeout=540s \
  --max-instances=5 \
  --set-env-vars="^;^GCP_PROJECT=${PROJECT_ID};GCP_REGION=${REGION};BQ_APP_DATASET=12_data_agent_log;BQ_APP_TABLE_PREFIX=raw_dashboard_builder_;CATALOG_GCS_BUCKET=surya-495408-dashboard-builder-catalog;EMBEDS_GCS_BUCKET=surya-495408-dashboard-builder-embeds;UPLOAD_WHITELIST_EMAILS=ngadt@hapas.vn,ductm@hapas.vn,thint@hapas.vn,dothunga1620@gmail.com;SERVING_DATASETS=00_serving_sales,00_serving_inventory,00_serving_operation;LARK_BASE_TOKEN=AcITbzsvraObhisDdQXlO6tggkd;LARK_TABLE_CATALOG_ID=tblA61jccZSexp2F;LARK_TABLE_KPI_ID=tblNr2LJO47ClgWW;LARK_OAUTH_HOST=https://open.larksuite.com;CLAUDE_MODEL=claude-sonnet-5;LARK_APP_ID=${LARK_APP_ID:?Set LARK_APP_ID env var trước khi chạy (không phải secret nhưng cũng không nên hardcode trong script commit lên git)}" \
